...
class mine_cell(tkinter.Label):
    def __init__(self,master,coord,number,cells,numBombs,bomb_counter):
        tkinter.Label.__init__(self,master,height=1,width=2,text='',bg='white',font=('Arial',24),relief='raised')
        self.color_map = colormap = ['black','blue','darkgreen','red','purple','maroon','cyan','black','dim gray']
        self.coord = coord
        self.number = number
        self.master = master
        self.cells = cells
    ...
    def auto_expose(self):
        try:
            if self.cells[(self.coord[0],self.coord[1]+1)].actual_num == 0 and self.cells[(self.coord[0],self.coord[1]+1)].bolea != True and self.cells[(self.coord[0],self.coord[1]+1)].get_number() != 'bomb':
        ...
        try:
            if self.cells[(self.coord[0]+1,self.coord[1])].actual_num == 0 and self.cells[(self.coord[0]+1,self.coord[1])].bolea != True and self.cells[(self.coord[0]+1,self.coord[1])].get_number() != 'bomb':
        ...
        try:
            ...
            pass
        try:
            if self.cells[(self.coord[0]-1,self.coord[1]-1)].actual_num == 0 and self.cells[(self.coord[0]-1,self.coord[1]-1)].bolea != True and self.cells[(self.coord[0]-1,self.coord[1]-1)].get_number() != 'bomb':
